...
ARTIFACTS_DIR="${AOT_MAF_HTTP_TOOL_ARTIFACTS_DIR:-$ROOT_DIR/artifacts/aot-maf-http-tool}"
RUNTIME_ID="${AOT_MAF_RUNTIME_IDENTIFIER:-$(dotnet --info | awk -F': ' '/ RID: / {print $2; exit}' | xargs)}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/openclaw-aot-maf-http-tool.XXXXXX")"
KEEP_WORK_DIR="${KEEP_AOT_MAF_HTTP_TOOL_DIR:-0}"
GATEWAY_PORT="${AOT_MAF_HTTP_TOOL_GATEWAY_PORT:-19939}"
PROVIDER_PORT="${AOT_MAF_HTTP_TOOL_PROVIDER_PORT:-21939}"
NOTE_KEY="aot-smoke"
...
GATEWAY_CONFIG="$WORK_DIR/gateway.maf.aot.http-tool.json"
...
echo "Publishing NativeAOT gateway with HTTP tool smoke support for $RUNTIME_ID..."
...
"$GATEWAY_BIN" --config "$GATEWAY_CONFIG" --doctor >/tmp/openclaw-aot-maf-http-tool-doctor.log 2>&1
...
echo "AOT MAF HTTP tool smoke passed."
