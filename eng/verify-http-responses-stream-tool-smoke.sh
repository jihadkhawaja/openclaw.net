...
ARTIFACTS_DIR="${HTTP_RESPONSES_STREAM_TOOL_SMOKE_ARTIFACTS_DIR:?HTTP_RESPONSES_STREAM_TOOL_SMOKE_ARTIFACTS_DIR is required}"
RUNTIME_MODE="${HTTP_RESPONSES_STREAM_TOOL_SMOKE_RUNTIME_MODE:?HTTP_RESPONSES_STREAM_TOOL_SMOKE_RUNTIME_MODE is required}"
ORCHESTRATOR="${HTTP_RESPONSES_STREAM_TOOL_SMOKE_ORCHESTRATOR:?HTTP_RESPONSES_STREAM_TOOL_SMOKE_ORCHESTRATOR is required}"
GATEWAY_PORT="${HTTP_RESPONSES_STREAM_TOOL_SMOKE_GATEWAY_PORT:?HTTP_RESPONSES_STREAM_TOOL_SMOKE_GATEWAY_PORT is required}"
PROVIDER_PORT="${HTTP_RESPONSES_STREAM_TOOL_SMOKE_PROVIDER_PORT:?HTTP_RESPONSES_STREAM_TOOL_SMOKE_PROVIDER_PORT is required}"
PUBLISH_AOT="${HTTP_RESPONSES_STREAM_TOOL_SMOKE_PUBLISH_AOT:-0}"
ENABLE_MAF="${HTTP_RESPONSES_STREAM_TOOL_SMOKE_ENABLE_MAF:-0}"
KEEP_WORK_DIR="${KEEP_HTTP_RESPONSES_STREAM_TOOL_SMOKE_DIR:-0}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/openclaw-http-responses-stream-tool-${RUNTIME_MODE}-${ORCHESTRATOR}.XXXXXX")"
...
GATEWAY_STREAM="$WORK_DIR/gateway-responses-stream.sse"
...
GATEWAY_CONFIG="$WORK_DIR/gateway.responses.stream-tool.json"
...
if [[ "$PUBLISH_AOT" == "1" ]]; then
  ...
  echo "Publishing NativeAOT gateway for Responses stream-tool smoke ($RUNTIME_MODE/$ORCHESTRATOR) on $RUNTIME_ID..."
  publish_args=(
    -c Release
    -r "$RUNTIME_ID"
  )
else
  echo "Publishing JIT gateway for Responses stream-tool smoke ($RUNTIME_MODE/$ORCHESTRATOR)..."
  publish_args=(
    -c Release
    -p:PublishAot=false
  )
...
echo "Issuing Responses API streaming tool request through the published gateway..."
...
python3 "$ROOT_DIR/eng/verify_response_stream_tool_result.py" \
...
echo "Responses HTTP stream-tool smoke passed for $RUNTIME_MODE/$ORCHESTRATOR."
