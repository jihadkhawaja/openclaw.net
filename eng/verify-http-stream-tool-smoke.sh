...
ARTIFACTS_DIR="${HTTP_STREAM_TOOL_SMOKE_ARTIFACTS_DIR:?HTTP_STREAM_TOOL_SMOKE_ARTIFACTS_DIR is required}"
RUNTIME_MODE="${HTTP_STREAM_TOOL_SMOKE_RUNTIME_MODE:?HTTP_STREAM_TOOL_SMOKE_RUNTIME_MODE is required}"
ORCHESTRATOR="${HTTP_STREAM_TOOL_SMOKE_ORCHESTRATOR:?HTTP_STREAM_TOOL_SMOKE_ORCHESTRATOR is required}"
GATEWAY_PORT="${HTTP_STREAM_TOOL_SMOKE_GATEWAY_PORT:?HTTP_STREAM_TOOL_SMOKE_GATEWAY_PORT is required}"
PROVIDER_PORT="${HTTP_STREAM_TOOL_SMOKE_PROVIDER_PORT:?HTTP_STREAM_TOOL_SMOKE_PROVIDER_PORT is required}"
PUBLISH_AOT="${HTTP_STREAM_TOOL_SMOKE_PUBLISH_AOT:-0}"
ENABLE_MAF="${HTTP_STREAM_TOOL_SMOKE_ENABLE_MAF:-0}"
KEEP_WORK_DIR="${KEEP_HTTP_STREAM_TOOL_SMOKE_DIR:-0}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/openclaw-http-stream-tool-${RUNTIME_MODE}-${ORCHESTRATOR}.XXXXXX")"
...
EXPECTED_TOOL_CHUNKS_JSON='["a","b","c"]'
EXPECTED_TOOL_RESULT="abc"
EXPECTED_FINAL_TEXT="Tool streamed: abc"
...
GATEWAY_STREAM="$WORK_DIR/gateway-stream.sse"
...
echo "Starting fake OpenAI-compatible streaming tool provider on port $PROVIDER_PORT..."
python3 "$ROOT_DIR/eng/fake_openai_tool_provider.py" \
  ...
  --stream-final-chunks-json '["Tool streamed: ","{tool_result}"]' \
  --stream-delay-ms 10 \
...
GATEWAY_CONFIG="$WORK_DIR/gateway.http.stream-tool.json"
...
if [[ "$PUBLISH_AOT" == "1" ]]; then
  ...
  echo "Publishing NativeAOT gateway for HTTP stream-tool smoke ($RUNTIME_MODE/$ORCHESTRATOR) on $RUNTIME_ID..."
  publish_args=(
    -c Release
    -r "$RUNTIME_ID"
  )
else
  echo "Publishing JIT gateway for HTTP stream-tool smoke ($RUNTIME_MODE/$ORCHESTRATOR)..."
  publish_args=(
    -c Release
    -p:PublishAot=false
  )
...
echo "Issuing OpenAI-compatible streaming tool request through the published gateway..."
REQUEST_START_MS="$(now_ms)"
curl --silent --fail --no-buffer \
  -H "Content-Type: application/json" \
  -d '{"model":"fake-maf-model","stream":true,"messages":[{"role":"user","content":"Use the stream_echo tool and then summarize it."}]}' \
...
curl --silent --fail "http://127.0.0.1:$GATEWAY_PORT/admin/events?component=llm&channelId=openai-http&limit=40" >"$ADMIN_EVENTS"

python3 "$ROOT_DIR/eng/verify_http_stream_tool_result.py" \
  ...
  --expected-tool-chunks-json "$EXPECTED_TOOL_CHUNKS_JSON" \
  --expected-tool-result "$EXPECTED_TOOL_RESULT" \
  --expected-final-text "$EXPECTED_FINAL_TEXT" \
...
echo "HTTP stream-tool smoke passed for $RUNTIME_MODE/$ORCHESTRATOR."
