...
def build_comparison_line(runtime_mode, native_report, maf_report):
    ...
    parts = [
        ...
        f"stream-tool turn `{format_ms(maf_turn)}` ms vs `{format_ms(native_turn)}` ms",
        ...
        f"tool results `{len(maf_stream['toolResults'])}` vs `{len(native_stream['toolResults'])}`",
    ]
...
def build_markdown(reports):
        ...
        rows.append(
            ...
            + " | ".join(
                [
                    ...
                    str(len(report["stream"]["toolResults"])),
    ...
    lines = [
        ...
        "",
        f"Generated on {datetime.now().astimezone().isoformat()} from the published-binary HTTP/SSE stream-tool matrix.",
        "",
        "## Matrix",
        "",
