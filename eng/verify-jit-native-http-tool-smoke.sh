...
ARTIFACTS_DIR="${JIT_NATIVE_HTTP_TOOL_ARTIFACTS_DIR:-$ROOT_DIR/artifacts/jit-native-http-tool}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/openclaw-jit-native-http-tool.XXXXXX")"
KEEP_WORK_DIR="${KEEP_JIT_NATIVE_HTTP_TOOL_DIR:-0}"
GATEWAY_PORT="${JIT_NATIVE_HTTP_TOOL_GATEWAY_PORT:-19959}"
PROVIDER_PORT="${JIT_NATIVE_HTTP_TOOL_PROVIDER_PORT:-21959}"
NOTE_KEY="jit-native-smoke"
NOTE_CONTENT="from published native jit"
...
GATEWAY_CONFIG="$WORK_DIR/gateway.native.jit.http-tool.json"
...
echo "Publishing JIT native gateway with HTTP tool smoke support..."
...
echo "Running JIT native gateway --doctor..."
"$GATEWAY_BIN" --config "$GATEWAY_CONFIG" --doctor >/tmp/openclaw-jit-native-http-tool-doctor.log 2>&1

echo "Starting JIT native gateway..."
...
echo "Issuing OpenAI-compatible request through the published JIT native gateway..."
...
echo "JIT native HTTP tool smoke passed."
