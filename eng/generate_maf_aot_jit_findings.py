...
def build_comparison_line(runtime_mode, native_report, maf_report):
    ...
    parts = [
        ...
        f"HTTP turn `{format_ms(maf_turn)}` ms vs `{format_ms(native_turn)}` ms",
...
def build_markdown(reports):
    ...
    lines = [
        ...
        "",
        f"Generated on {datetime.now().astimezone().isoformat()} from the published-binary HTTP/tool matrix.",
        "",
        "## Matrix",
        "",
