...
ARTIFACTS_DIR="${JIT_MAF_HTTP_TOOL_ARTIFACTS_DIR:-$ROOT_DIR/artifacts/jit-maf-http-tool}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/openclaw-jit-maf-http-tool.XXXXXX")"
KEEP_WORK_DIR="${KEEP_JIT_MAF_HTTP_TOOL_DIR:-0}"
GATEWAY_PORT="${JIT_MAF_HTTP_TOOL_GATEWAY_PORT:-19949}"
PROVIDER_PORT="${JIT_MAF_HTTP_TOOL_PROVIDER_PORT:-21949}"
NOTE_KEY="jit-smoke"
NOTE_CONTENT="from published maf jit"
...
GATEWAY_CONFIG="$WORK_DIR/gateway.maf.jit.http-tool.json"
...
echo "Publishing JIT gateway with HTTP tool smoke support..."
...
echo "Running JIT gateway --doctor..."
"$GATEWAY_BIN" --config "$GATEWAY_CONFIG" --doctor >/tmp/openclaw-jit-maf-http-tool-doctor.log 2>&1

echo "Starting JIT gateway..."
...
echo "Issuing OpenAI-compatible request through the published JIT gateway..."
...
echo "JIT MAF HTTP tool smoke passed."
