...
ARTIFACTS_DIR="${HTTP_STREAM_SMOKE_ARTIFACTS_DIR:?HTTP_STREAM_SMOKE_ARTIFACTS_DIR is required}"
RUNTIME_MODE="${HTTP_STREAM_SMOKE_RUNTIME_MODE:?HTTP_STREAM_SMOKE_RUNTIME_MODE is required}"
ORCHESTRATOR="${HTTP_STREAM_SMOKE_ORCHESTRATOR:?HTTP_STREAM_SMOKE_ORCHESTRATOR is required}"
GATEWAY_PORT="${HTTP_STREAM_SMOKE_GATEWAY_PORT:?HTTP_STREAM_SMOKE_GATEWAY_PORT is required}"
PROVIDER_PORT="${HTTP_STREAM_SMOKE_PROVIDER_PORT:?HTTP_STREAM_SMOKE_PROVIDER_PORT is required}"
STREAM_CHUNKS_JSON="${HTTP_STREAM_SMOKE_CHUNKS_JSON:?HTTP_STREAM_SMOKE_CHUNKS_JSON is required}"
EXPECTED_STREAM_TEXT="${HTTP_STREAM_SMOKE_EXPECTED_TEXT:?HTTP_STREAM_SMOKE_EXPECTED_TEXT is required}"
PUBLISH_AOT="${HTTP_STREAM_SMOKE_PUBLISH_AOT:-0}"
ENABLE_MAF="${HTTP_STREAM_SMOKE_ENABLE_MAF:-0}"
KEEP_WORK_DIR="${KEEP_HTTP_STREAM_SMOKE_DIR:-0}"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/openclaw-http-stream-${RUNTIME_MODE}-${ORCHESTRATOR}.XXXXXX")"
...
echo "Starting fake OpenAI-compatible streaming provider on port $PROVIDER_PORT..."
python3 "$ROOT_DIR/eng/fake_openai_tool_provider.py" \
  --port "$PROVIDER_PORT" \
  --log "$FAKE_PROVIDER_LOG" \
  --model "fake-maf-model" \
  --stream-chunks-json "$STREAM_CHUNKS_JSON" \
  --stream-delay-ms 10 \
...
GATEWAY_CONFIG="$WORK_DIR/gateway.http.stream.json"
...
if [[ "$PUBLISH_AOT" == "1" ]]; then
  ...
  echo "Publishing NativeAOT gateway for HTTP stream smoke ($RUNTIME_MODE/$ORCHESTRATOR) on $RUNTIME_ID..."
  publish_args=(
    -c Release
    -r "$RUNTIME_ID"
  )
else
  echo "Publishing JIT gateway for HTTP stream smoke ($RUNTIME_MODE/$ORCHESTRATOR)..."
  publish_args=(
    -c Release
    -p:PublishAot=false
  )
...
echo "Issuing OpenAI-compatible streaming request through the published gateway..."
REQUEST_START_MS="$(now_ms)"
curl --silent --fail --no-buffer \
  -H "Content-Type: application/json" \
  -d '{"model":"fake-maf-model","stream":true,"messages":[{"role":"user","content":"Stream a response in two chunks."}]}' \
...
python3 "$ROOT_DIR/eng/verify_http_stream_result.py" \
  ...
  --expected-content "$EXPECTED_STREAM_TEXT" \
  --expected-chunks-json "$STREAM_CHUNKS_JSON" \
...
echo "HTTP stream smoke passed for $RUNTIME_MODE/$ORCHESTRATOR."
