...
def build_comparison_line(runtime_mode, native_report, maf_report):
    ...
    parts = [
        ...
        f"responses stream-tool turn `{format_ms(maf_turn)}` ms vs `{format_ms(native_turn)}` ms",
        ...
        f"function_call_output items `{len(maf_stream['toolOutputItems'])}` vs `{len(native_stream['toolOutputItems'])}`",
    ]
    ...
    if maf_stream["fullText"] == native_stream["fullText"]:
        parts.append(f"final text matched: `{maf_stream['fullText']}`")
    else:
        parts.append(
            "final text differed: "
...
def build_markdown(reports):
        ...
        rows.append(
            ...
            + " | ".join(
                [
                    ...
                    str(len(report["stream"]["toolCalls"])),
    ...
    lines = [
        ...
        "",
        f"Generated on {datetime.now().astimezone().isoformat()} from the published-binary `/v1/responses` stream-tool matrix.",
        "",
        "## Matrix",
        "",
